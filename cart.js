  ...
  // Hide all by default
  document.getElementById("scanpayFinalAmount").style.display = "none";
  ...
  if (razorPay.checked) {
    ...
    totalPriceEl.innerHTML = isCartEmpty ? "" : `Total: ₹${cartTotal}/-`;

    if (!isCartEmpty) {
      const finalAmt = `Final Amount to Pay: ₹${cartTotal}/-`;
      document.getElementById("razorpayFinalAmount").innerText = finalAmt;
    ...
    if (!isCartEmpty) {
      const finalAmt = `Final Amount to Pay: ₹${cartTotal + 100}/- (Includes ₹100 COD charge)`;
      document.getElementById("codFinalAmount").innerText = finalAmt;
      document.getElementById("codFinalAmount").style.display = "block";
    }

  } else if (scanPay.checked) {
    ...
    totalPriceEl.innerHTML = isCartEmpty ? "" : `Total: ₹${cartTotal + 50}/-`;

    if (!isCartEmpty) {
      const finalAmt = `Final Amount to Pay: ₹${cartTotal + 50}/- (Includes ₹50 Delivery Partner fee)`;
      document.getElementById("scanpayFinalAmount").innerText = finalAmt;
      document.getElementById("scanpayFinalAmount").style.display = "block";
    }
  }
}
  ...
  scanPay.addEventListener("change", updateDisplayAndButtons);
  ...
  calculateCartTotal();
  const finalAmount = cartTotal; // No extra charge for Razorpay

  // Show final amount in Razorpay section
  ...
  document.getElementById("orderForm").addEventListener("submit", function (e) {
    ...
    if (!scanPay.checked && !codPay.checked && !razorPay.checked) {
